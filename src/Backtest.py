            ...
            for idxr, risk in enumerate(risk_samples):
                ...
                for idxl, leverage in enumerate(leverage_samples):
                    ...
                    for idxrr, rr in enumerate(risk_reward):
                        ...
                        axs[idx_subplot].plot(sample, color='y', alpha=0.05)
                        axs[idx_subplot].plot(mean_percentile, color='r')
                        axs[idx_subplot].plot(upper_percentile, color='r')
                        axs[idx_subplot].plot(lower_percentile, color='r')
                        axs[idx_subplot].plot(rtit.equity_curve, color='b')

                        axs[idx_subplot].set_ylim([0, 1.1*upper_percentile.max()])

                        axs[idx_subplot].set_title(
                            ...
                            + 'Musch: {:.2f}'.format(stats_dict['musch']))
                        axs[idx_subplot].grid(True)
